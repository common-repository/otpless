...
class Otpless_Login
{
    ...
    public function __construct()
    {
        ...
        if (empty($otpless_options)){
            ...
            $otpless_options['checkout_login'] = 0;
        }
       
        $this->checkout = $otpless_options['checkout_login'];
        ...
        if(!empty($this->cSecret) && !empty($this->cId)){
            ...
            add_shortcode( 'otpless_button_signin', array($this, 'otpless_button_drag_and_drop_button_shortcode'));
            add_action( 'init', array( $this, 'otpless_authorize_user' ), 10 );
        } 
    }

    function otpless_button_drag_and_drop_button_shortcode(){
		if(!is_user_logged_in()){
            ...
            <div class="overlay dsp-n" id="overlay" onclick="togglePopup()">
                <div class="popup">
                    ...
                    <script defer src="https://otpless.com/v2/wordpress.js" id="otpless-sdk" cid="'.$this->cId.'" data-appid="'.$this->appId.'"></script>
                </div>
            </div>
            <script>
...
<?php
    ...
    function importScript() {
        $template = '<script defer src="https://otpless.com/v2/wordpress.js" data-appid="'.$this->appId.'" cid="'.$this->cId.'" id="otpless-sdk"></script>';
        echo $template;
    }
    ...
    function otpless_login_page_drag_and_drop_button_shortcode() {
        if(!is_user_logged_in()){
                $html = '<div id="otpless-login-page" ></div><script defer src="https://otpless.com/v2/wordpress.js" data-appid="'.$this->appId.'" cid="'.$this->cId.'"  id="otpless-sdk"></script>';
          return $html;
        }else{
            $html = '';
        }
    ...
    public function otpless_create_user($mobile, $name, $email){
        ...
        if(!$email == '' && !$email == null && !$mobile == '' && !$mobile == null){
            ...
            $result = wp_create_user($mobile, wp_generate_password(), "");
            if(is_wp_error($result)){ // If get error to create the user - log message
                $error = $result->get_error_message();
                $this->log( $error );
            }else{ // If user was successfully created - receive and return login url
